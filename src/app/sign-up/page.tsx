...
type Step = 'email' | 'name' | 'photo' | 'instagram'
const STEPS: Step[] = ['email', 'name', 'photo', 'instagram']
...
export default function SignUpPage() {
  ...
  async function uploadAvatar(file: File) {
    ...
    try {
      ...
      if (storageErr) {
        setUploadError('upload failed — make sure the storage bucket is set up.')
        return
      }
  ...
  async function handleInstagramConnect() {
    ...
    const url = `${supabaseUrl}/functions/v1/instagram-oauth/authorize?apikey=${supabaseAnonKey}&platform=web&redirect_to=${encodeURIComponent(redirectTo)}&token=${encodeURIComponent(session.access_token)}`
  ...
  const primaryBtn = "w-full rounded-full bg-parchment text-ink text-sm font-medium py-3 hover:bg-parchment/90 transition disabled:opacity-40"
  const ghostBtn = "w-full rounded-full border border-parchment/20 text-parchment/70 text-sm py-3 hover:border-parchment/40 hover:text-parchment transition"
      ...
      <div className="relative z-10 w-full max-w-sm px-6 py-12">
        ...
        <AnimatePresence mode="wait" custom={dir}>
          ...
          {step === 'email' && (
            ...
            >
              <div className="text-center flex flex-col gap-1">
                ...
                <p className="text-sm text-chalk/60">join to post quests and find your errand buddy.</p>
              </div>
              ...
              {confirmEmail ? (
                <div className="rounded-2xl border border-parchment/10 bg-parchment/5 px-5 py-5 text-center flex flex-col gap-2">
                  ...
                  <p className="text-xs text-chalk/60 leading-relaxed">
                    we sent a confirmation link to <span className="text-parchment/80">{email}</span>. click it to activate your account, then sign in.
                  </p>
                  <Link href="/sign-in" className="mt-2 text-xs text-parchment/60 underline underline-offset-2 hover:text-parchment transition">
                    go to sign in
                  </Link>
                </div>
              ) : (
                <form onSubmit={handleSignUp} className="flex flex-col gap-3">
                  ...
                  <div className="relative">
                    <input
                      ...
                      className="w-full bg-parchment/5 border border-parchment/15 rounded-full py-3 pl-5 pr-12 text-sm text-parchment placeholder:text-chalk/30 focus:outline-none focus:border-parchment/40 transition"
                    />
                    <button
                      type="button"
                      onClick={() => setShowPassword(p => !p)}
                      className="absolute right-4 top-1/2 -translate-y-1/2 text-parchment/30 hover:text-parchment/60 transition"
                      tabIndex={-1}
                    >
                      {showPassword ? (
              ...
              <p className="text-center text-xs text-chalk/40">
                ...
                <Link href="/sign-in" className="text-parchment/70 underline underline-offset-2 hover:text-parchment transition">
                  sign in
                </Link>
              </p>
            </motion.div>
          ...
          {step === 'name' && (
            ...
            >
              <div className="text-center flex flex-col gap-1">
                <h1 className="font-serif italic text-3xl text-parchment">what should we call you?</h1>
                <p className="text-sm text-chalk/60">this is what other questers will see.</p>
              </div>
          ...
          {step === 'photo' && (
            ...
            >
              <div className="text-center flex flex-col gap-1">
                ...
                <p className="text-sm text-chalk/60">optional — helps people recognise you.</p>
              </div>
              ...
              <div className="flex justify-center">
                ...
                >
                  {avatarUrl ? (
                    ...
                    <span className="text-4xl text-parchment/20 select-none">
                      {uploading ? '…' : '+'}
                    </span>
                  )}
                </button>
          ...
          {step === 'instagram' && (
            ...
            >
              <div className="text-center flex flex-col gap-2">
                <h1 className="font-serif italic text-3xl text-parchment">connect instagram.</h1>
                <p className="text-sm text-chalk/60 leading-relaxed">
                  we read your posts to build a 74-point taste fingerprint that powers your match score. we never post or store your photos.
                </p>
              </div>
